...
const ProgressBar = ({ activeFieldset }) => (
  <ul id="progressbar">
    ...
    <li className={activeFieldset >= 2 ? "active completed" : ""}>SCRAPING</li>
    <li className={activeFieldset >= 3 ? "active completed" : ""}>
      EXTRACTED DATA
    </li>
  </ul>
...
const App = () => {
  ...
  const [checkedUrlsArray, setCheckedUrlsArray] = useState([]);
  const [isScrapingClicked, setIsScrapingClicked] = useState(false);
  ...
  const handleCheckboxChange = async (url, id) => {
    ...
    await setCheckedUrls(newCheckedUrls);

    try {
      const urlsArray = Object.keys(newCheckedUrls).filter(
        (url) => newCheckedUrls[url]
      );
      console.log("Checked URLs:", urlsArray);

      await axios.put(`${BASE_URL}/api/update-ischecked`, {
        url,
        id,
        isChecked: newCheckedUrls[url],
        urlsArray,
      });
      // await axios.put(`${BASE_URL}/api/update-ischecked`, {url, id, isChecked: checkedUrls[url], urlsArray});
      console.log(`Updated isChecked for ${url}`);
    } catch (error) {
      console.error("Error updating isChecked:", error);
    }
  ...
  const fetchData1 = async () => {
    try {
      axios
        .get(`http://localhost:5002/api/search?BASE_URL=${url.trim()}`)
        // .get(`${BASE_URL}/api/search?BASE_URL=${url.trim()}`)
        .then((res) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("Response:", response);
      fetchData1();
      // Setting isScrapingClicked to true after initiating scraping
      setIsScrapingClicked(true);
      ...
      setDisplayUrls(true);
      alert("Scraping process initiated! Please wait. ");
    } catch (err) {
      console.log("app", err);
    } finally {
      setLoading(false);
  ...
  const handleNext = () => {
    const checkedUrlsArray = Object.keys(checkedUrls).filter(
      (url) => checkedUrls[url]
    );

    if (checkedUrlsArray.length === 0) {
      alert("Please select at least one URL.");
    } else {
      setActiveFieldset((prevActiveFieldset) => prevActiveFieldset + 1);
      setCheckedUrlsArray(checkedUrlsArray);
    }
  };

  const handleNext2 = () => {
    setActiveFieldset((prevActiveFieldset) => prevActiveFieldset + 1);
    // setCheckedUrlsArray(Object.keys(checkedUrls).filter(url => checkedUrls[url]));
  };
  ...
  const handleScrapeData = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:5002/scrape-data', {
        selectors,
      });
      // const response = await axios.post(`${BASE_URL}/scrape-data`, { selector });
      console.log("response scrape-data", response.data);
      setScrapedData(Array.from(response.data));
    } catch (error) {
  ...
  return (
    <div>
      ...
      <form id="msform" onSubmit={handleSubmit}>
        <Fieldset
          style={{ display: activeFieldset === 1 ? "block" : "none" }}
          className="complete"
        >
          <h2 className="fs-title">Provide the Main URL</h2>
          <h3 className="fs-subtitle">
          ...
          <button
            ...
            className="next action-button"
            onClick={handleNext}
            disabled={!url || showUrlWarning || !isScrapingClicked} // Add condition here
          >
            Next
          </button>
          {displayUrls && (
        ...
        <Fieldset style={{ display: activeFieldset === 2 ? "block" : "none" }}>
          ...
          <div>
            {checkedUrlsArray.map((url, index) => (
              <div
                className="url-list2"
                key={index}
